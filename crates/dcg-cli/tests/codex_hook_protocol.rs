...
pub fn run_hook_raw_with_config(
    ...
    cmd.env_clear()
        ...
        .env("HOME", home.path())
        .env("USERPROFILE", home.path())
        // Point the loader at the hermetic config dir deterministically on every
        // platform. Without this, `~/.config/dcg` resolution depends on the
        // platform-native config dir (e.g. %APPDATA% on Windows) and the config
        // written above is not found, which turns ask/warn policy tests into
        // default-denies.
        .env("XDG_CONFIG_HOME", home.path().join(".config"))
        .env("TMPDIR", &tmp_path)
        .env("TEMP", &tmp_path)
...
fn run_with_user_allowlist(allowlist_toml: &str, command: &str, use_codex: bool) -> HookOutcome {
    ...
    cmd.env_clear()
        ...
        .env("HOME", home.path())
        .env("USERPROFILE", home.path())
        // Point the allowlist loader at the hermetic config dir (see the
        // run_hook_raw_with_config comment): without XDG_CONFIG_HOME, Windows
        // resolves ~/.config to %APPDATA% and the allowlist is not found.
        .env("XDG_CONFIG_HOME", home.path().join(".config"))
        .env("TMPDIR", home.path().join("tmp"))
        .env("TEMP", home.path().join("tmp"))
        .env("TMP", home.path().join("tmp"))
